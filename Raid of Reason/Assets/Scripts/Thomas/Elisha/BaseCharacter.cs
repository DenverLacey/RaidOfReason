    ...
    public virtual void TakeDamage(float damage)
    {
        ...
        if (m_currentHealth <= 0.0f)
        {
            ...
            if (playerState != PlayerState.DEAD)
            {
                ...
                if (CharacterType == CharacterType.KENRON)
                {
                    AkSoundEngine.PostEvent("Kenron_Death_Event", gameObject);
                }
                else if (CharacterType == CharacterType.KREIGER)
                {
                    AkSoundEngine.PostEvent("Machina_Death_Event", gameObject);
                }
                else
                {
                    AkSoundEngine.PostEvent("Thea_Death_Event", gameObject);
                }
            }
