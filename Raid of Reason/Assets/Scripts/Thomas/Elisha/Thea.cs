...
public class Thea : BaseCharacter 
{
	...
	private void Start()
	{
        ...
        skillManager.m_mainSkills[2].onDone += OnSkillReady;
	...
	public void Projectile()
    {
		...
		if (XCI.GetAxis(XboxAxis.RightTrigger, controller) > 0.1f && !isSkillActive)
		{
			...
			if (m_counter >= m_projectileDelay)
			{
				...
				m_counter = 0;
                //Ak SoundEngine Grab
                AkSoundEngine.PostEvent("Thea_Attack_Event", gameObject);
            }
		}
		else
		{
    ...
    {
        GOPRadiusIndicator.GetComponent<AkEvent>().enabled=true;
	...
	public void GiftOfPoseidonHealAndReset()
	{
        ...
        m_HealRadius1.Play();
        //Ak SoundEngine Grab
        AkSoundEngine.PostEvent("Thea_Ability_Close_Event", gameObject);


        AkSoundEngine.SetRTPCValue("Thea_Close", 100);
        Invoke("resetRTPC", 8);

        //AkSoundEngine.SetRTPCValue("Thea_Close", 1);
        //GOPRadiusIndicator.GetComponent<AkEvent>().enabled = false;
    }
        public void resetRTPC()
    {
        AkSoundEngine.SetRTPCValue("Thea_Close", 1);
    }
    /// <summary>
    ...
    private void OnSkillReady()
    {
        ...
        AkSoundEngine.PostEvent("Thea_UI_CoolDowns_Event", gameObject);
    }
}